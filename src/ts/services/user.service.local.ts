...
function loadUsersDemoData(demoHost: UserProps) {
    ...
    demoHost.listings.map(listing =>
        listing.reservations.forEach(listingReservation => {
            ...
            const nightsCount =
                utilService.getNightsCount({
                    checkIn: new Date(listingReservation.dates.checkIn),
                    checkOut: new Date(listingReservation.dates.checkOut),
                }) || 2
            const totalPayout = listing.price * nightsCount
            const reservation = {
                _id: listingReservation._id,
                stayId: listing._id,
                ...
                reservationDates: listingReservation.dates,
            ...
            guest.tripsId.push(reservation._id)
            reservationService.addDemoReservation(reservation)
